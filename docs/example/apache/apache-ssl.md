...
- This forwards all the app's endpoints (`/slack/events`, `/slack/commands`, `/slack/actions`, `/slack/install`, `/slack/oauth_redirect`), so the `https://YOURHOSTNAME/slack/...` request URLs from [self_host.md](../linux/self_host.md) work as-is.
