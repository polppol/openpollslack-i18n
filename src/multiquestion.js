...
function buildBlocks(pollData, votesDoc, opts = {}) {
  ...
  // Delete is destructive + creator-gated in handleMenu. NOTE: a `confirm` object is NOT
  // valid on a select OPTION (Slack's option schema has no confirm — it belongs on the
  // ELEMENT). Putting one here rendered in modals but made Slack reject the POSTED message
  // with HTTP 500 ("invalid_blocks") via response_url. handleMenu opens a confirm MODAL for
  // delete instead (mirrors the single poll, which also confirms via a modal, not an option).
